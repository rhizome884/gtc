...
X_test, y_test, clips = prepare_test_set('test-sets/test-synthetic.json')
...
for i, pred in enumerate(y_preds):
    ...
    clip = clips[i].split('_decoded')[0]
    if i+1 != len(y_preds):
        clip_next = clips[i+1].split('_decoded')[0]
    else:
